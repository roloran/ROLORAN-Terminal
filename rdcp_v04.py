...
RDCP_MSGTYPE_RESET_INFRASTRUCTURE = 0x12
RDCP_MSGTYPE_EMERGENCY = 0x1A
RDCP_MSGTYPE_CITIZEN_REQUEST = 0x1B
...
def rdcp_create_message(
    ...
    counter=0x00,  # retransmission and hop counter combination, 0 by default
    relay1=0xF0,  # relay schedule 1
    relay2=0xF0,  # relay schedule 2
    relay3=0xF0,  # relay schedule 3
    crc=0x0000,  # CRC-16
    payload=b"",
):
...
def rdcp_message_pretty_print(m):
    ...
    print()
    print("To use this message with a T-Deck via Serial, try either of:")
...
def rdcp_create_OA_payload(
    ...
    more_fragments=0,  # we don't use fragments yet ;-)
    text="",
):
