...
ROLORAN-Terminal supports the ALAAF paradigm and is A Lora As A File (ALAAF) implementation. This allows for sending and receiving LoRa packets and RDCP Messages via the file system.

In `roloran-terminal.py`, `enable_alaaf_write` and `enable_alaaf_read` can be set along with the file paths `alaaf_write_path` and `alaaf_read_path`. In the `write` directory, a new timestamped file with `.alaaf` extension is created including the RXMETA and RX lines of received LoRa packets. In the `read` directory, arbitrary Serial commands to be sent to the connected RDCP device may be stored in `.alaaf` files, which will be renamed to `.done` when processed. As usual for ALAAF implementations, preparing files and then renaming them to `.alaaf` is recommended to avoid race conditions. A third party process must be used to clean up `.done` files and unused `.alaaf` files as recommended in the ALAAF specification.

### LARS support

ROLORAN-Terminal supports the LARS paradigm and is a Lora As Remote Service (LARS) implementation that supports TCP/IP connections from multiple clients in parallel.

In `roloran-terminal.py`, `enable_lars`, `lars_ip`, and `lars_port` should be set accordingly. Note that incoming connections are not authenticated, so it is recommended to listen on a localhost address only and combine LARS, e.g., with SSH tunnels when used across networked systems.
